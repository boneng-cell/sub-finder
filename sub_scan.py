...
def enumerate_subdomains(domain):
    ...
    with open(os.path.join(domain, "all_subs.txt"), "w") as f:
...
def run_httpx(domain):
    ...
    all_subs_path = os.path.join(domain, "all_subs.txt")
    if not os.path.exists(all_subs_path):
        return False
    with open(all_subs_path, "r") as f:
        urls = f.read().splitlines()
    print(f"[*] Scanning {len(urls)} URLs with httpx")
    cmd = ["httpx", "-l", all_subs_path, "-json", "-silent"]
    process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True)
    files = {
        200: open(os.path.join(domain, "200.txt"), "w"),
        300: open(os.path.join(domain, "300.txt"), "w"),
        400: open(os.path.join(domain, "400.txt"), "w"),
        500: open(os.path.join(domain, "500.txt"), "w")
    }
    ...
    for code in [200,300,400,500]:
        filepath = os.path.join(domain, f"{code}.txt")
...
def main():
    ...
    if sub_count > 0:
        run_httpx(domain)
        print(f"[✓] Done: {domain}/all_subs.txt, {domain}/200.txt, {domain}/300.txt, {domain}/400.txt, {domain}/500.txt")
